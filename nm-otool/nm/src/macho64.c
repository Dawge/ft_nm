...
static int		find_addr(t_list_inf *sym, int sz)
{
	int			i;
	int			idx;
...
static void		print_list(t_list_inf *sym, int sz, int *tab)
{
	...
	while (i < sz)
	{
		...
		{
		//	if (ft_strcmp(sym[idx].str, "bad string index") == 0)
		//		printf("\noff[%03d] = %05u %u\n", idx, sym[idx].lst.n_desc, sym[idx].lst.n_un.n_strx);
			//printf("type = 0x%x\n", sym[idx].lst.n_type);
			if (type == 'I')
				printf("%18c %s (indirect for %s)\n", type, sym[idx].str, sym[idx].str);
			else if (sym[idx].lst.n_value == 0x0 && type != 'T' && type != 't' && type != 'A')
				printf("%18c %s\n", type, sym[idx].str);
			else
...
static int		sym_64(t_macho64 *inf, t_file_inf file, int offset, int *tab)
{
	...
	while (i < (int)inf->symtab.nsyms)
	{
		...
		{
			inf->symbol[i].oof = offset + inf->symtab.stroff + inf->symbol[i].lst.n_un.n_strx;
